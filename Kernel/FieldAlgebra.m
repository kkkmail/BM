...
(* ============================================== *)
Options[FieldAlgebra] =
    {
      FieldAlgebraVersion -> 6.04,
      FunctionName -> "Unknown",
      FunctionDescription -> "Unknown",
      NonAverageable -> False,
      NonAverageableIndex -> 1,
      UseAnalyzer -> False,
      AbsoluteAzimuth -> True,
      AzimuthRangeType -> 0,
      AzimuthShiftType -> 1,
      PrintFunctionDebugInfo -> False,
      PrintFunctionDebugInfoLevel -> 0,
      SetEllipticitySignR -> True,
      UseZAxisForEllipticityR -> False,
      SetEllipticitySignT -> True,
      UseZAxisForEllipticityT -> False,
      SetEllipticitySignI -> True,
      UseZAxisForEllipticityI -> False,
      EllipticityCutOffForAzimyth -> 0.9999,
      AveragingFuncCall -> None,
      RequiresCalculateBeta0and90 -> False
    };
(* ============================================== *)
FAAZTOLERANCE = 10^-6;
...
Options[LnRFull] = {FunctionName -> "Ln[RFull]", FunctionDescription -> "Natural logarithm of full intensity of Reflected light."};
Options[LnTFull] = {FunctionName -> "Ln[TFull]", FunctionDescription -> "Natural logarithm of full intensity of Transmitted light."};
Options[LnRx] = {FunctionName -> "Ln[Rx]", FunctionDescription -> "Natural logarithm of Intensity of Reflected light going into X component."};
...
Options[LnTx] = {FunctionName -> "Ln[Tx]", FunctionDescription -> "Natural logarithm of Intensity of Transmitted light going into X component."};
...
(* ============================================== *)
FunctionNameString[HoldPattern[EI[xyz_]]] :=
    Module[{retval},
      retval = "EI[" <> ToString[If[xyz === 1, "x", If[xyz === 2, "y", If[xyz === 3, "z"]]]] <> "]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[ER[xyz_]]] :=
    Module[{retval},
      retval = "ER[" <> ToString[If[xyz === 1, "x", If[xyz === 2, "y", If[xyz === 3, "z"]]]] <> "]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[ET[xyz_]]] :=
    Module[{retval},
      retval = "ET[" <> ToString[If[xyz === 1, "x", If[xyz === 2, "y", If[xyz === 3, "z"]]]] <> "]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[PoyntingI[xyz_]]] :=
    Module[{retval},
      retval = "SI[" <> ToString[If[xyz === 1, "x", If[xyz === 2, "y", If[xyz === 3, "z"]]]] <> "]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[PoyntingR[xyz_]]] :=
    Module[{retval},
      retval = "SR[" <> ToString[If[xyz === 1, "x", If[xyz === 2, "y", If[xyz === 3, "z"]]]] <> "]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[PoyntingT[xyz_]]] :=
    Module[{retval},
      retval = "ST[" <> ToString[If[xyz === 1, "x", If[xyz === 2, "y", If[xyz === 3, "z"]]]] <> "]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[PoyntingVector2[evi_, xyz_]]] :=
    Module[{retval},
      retval = "S2[" <> ToString[evi] <> "," <> ToString[If[xyz === 1, "x", If[xyz === 2, "y", If[xyz === 3, "z"]]]] <> "]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[PoyntingXY2[evi_]]] :=
    Module[{retval},
      retval = "Sxy2[" <> ToString[evi] <> "]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[PoyntingXYPhase2[evi_]]] :=
    Module[{retval},
      retval = "SxyPhase2[" <> ToString[evi] <> "]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[M2EValRe[evi_]]] :=
    Module[{retval},
      retval = "Re[Eval(M2)[" <> ToString[evi] <> "]]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[M2EValIm[evi_]]] :=
    Module[{retval},
      retval = "Im[Eval(M2)[" <> ToString[evi] <> "]]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[ReEGE2[evi_, xyz_]]] :=
    Module[{retval},
      retval = "Re[E2[" <> ToString[evi] <> "," <> ToString[If[xyz === 1, "x", If[xyz === 2, "y", If[xyz === 3, "z"]]]] <> "]]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[ImEGE2[evi_, xyz_]]] :=
    Module[{retval},
      retval = "Im[E2[" <> ToString[evi] <> "," <> ToString[If[xyz === 1, "x", If[xyz === 2, "y", If[xyz === 3, "z"]]]] <> "]]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[ReEGH2[evi_, xyz_]]] :=
    Module[{retval},
      retval = "Re[H2[" <> ToString[evi] <> "," <> ToString[If[xyz === 1, "x", If[xyz === 2, "y", If[xyz === 3, "z"]]]] <> "]]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[ImEGH2[evi_, xyz_]]] :=
    Module[{retval},
      retval = "Im[H2[" <> ToString[evi] <> "," <> ToString[If[xyz === 1, "x", If[xyz === 2, "y", If[xyz === 3, "z"]]]] <> "]]";
      Return[retval];
    ];

FunctionNameString[HoldPattern[EpsComponent[UpDown_, ReIm_, xyzRow_, xyzCol_]]] :=
    Module[{retval, hdr},
      hdr = "Eps";
...
FunctionNameString[HoldPattern[MuComponent[UpDown_, ReIm_, xyzRow_, xyzCol_]]] :=
    Module[{retval, hdr},
      hdr = "Mu";
      retval = If[UpDown > 0, "Layer " <> ToString[UpDown] <> ": ", ""] <> If[ReIm === 1, "Re", "Im"] <> "[" <> hdr <> If[UpDown === -1, "1", If[UpDown === -2, "2", ""]] <> "[" <> ToString[If[xyzRow === 1, "x", If[xyzRow === 2, "y", If[xyzRow === 3, "z"]]]] <> "," <> ToString[If[xyzCol === 1, "x", If[xyzCol === 2, "y", If[xyzCol === 3, "z"]]]] <> "]]";
      Return[retval];
...
FunctionNameString[HoldPattern[RoComponent[UpDown_, ReIm_, xyzRow_, xyzCol_]]] :=
    Module[{retval, hdr},
      hdr = "Ro";
      retval = If[UpDown > 0, "Layer " <> ToString[UpDown] <> ": ", ""] <> If[ReIm === 1, "Re", "Im"] <> "[" <> hdr <> If[UpDown === -1, "1", If[UpDown === -2, "2", ""]] <> "[" <> ToString[If[xyzRow === 1, "x", If[xyzRow === 2, "y", If[xyzRow === 3, "z"]]]] <> "," <> ToString[If[xyzCol === 1, "x", If[xyzCol === 2, "y", If[xyzCol === 3, "z"]]]] <> "]]";
      Return[retval];];
...
Return[retval];];
(* ============================================== *)
GetFunctionName[func_] :=
    Module[{strFuncName, opts, hd, lst},
      opts = Options[func];
      lst = Apply[List, func];
      (*Print["opts = ",opts];
      Print["func = ",func];
      Print["lst = ",lst];*)
      strFuncName = If[Head[lst] === Head[{}], FunctionNameString[func], FunctionName /. opts /. Options[FieldAlgebra]];
      (*Print["strFuncName = ",strFuncName];*)
      Return[strFuncName];
    ];

SetAttributes[GetFunctionName, Listable];
(* ============================================== *)
GetFunctionDesc[func_] :=
    Module[{strFuncDesc, opts, hd, lst, fnelhlp1, fnelhlp2, ueFuncListHlp1},
      lst = Apply[List, func];
      fnelhlp1 = Apply[List, func];
      fnelhlp2 = Head[func];
      ueFuncListHlp1 = If[Head[fnelhlp1] === Head[{}], fnelhlp2, fnelhlp1];
      (*Print["ueFuncListHlp1 = ",ueFuncListHlp1];*)
...
(* ============================================== *)
GetFunctionOptions[func_] :=
    Module[{opts, hd, lst, fnelhlp1, fnelhlp2, ueFuncListHlp1},
      lst = Apply[List, func];
      fnelhlp1 = Apply[List, func];
      fnelhlp2 = Head[func];
      ueFuncListHlp1 = If[Head[fnelhlp1] === Head[{}], fnelhlp2, fnelhlp1];
...
(* ============================================== *)
PoyntingVector[ehField : {_, _, _, _, _, _}] :=
...
PoyntingVector[ehField : {_, _, _, _, _, _, _}] :=
    Module[{retVal, eField, hField},
      eField = Table[ehField[[iii]], {iii, 1, 3}];
      hField = Table[ehField[[iii + 3]], {iii, 1, 3}];
      retVal = PoyntingVector[eField, hField];
...
PoyntingVector[eField : {_, _, _}, hField : {_, _, _}] :=
    Module[{retVal},
      retVal = Re[Flatten[eField]\[Cross]Flatten[Conjugate[hField]]];
      Return[retVal];
    ];
(* ============================================== *)
IFullBase[FullSol_] :=
...
(* ============================================== *)
IFull[FullSol_] :=
    Module[{Efield, Dfield, Hfield, Bfield, retVal, ExBvec, ExHvec, Ibase, ExH1vec, opts, pdi, pdil, ExHvecRe, ExEHvecRe}, o
      pts = GetSolOptions[FullSol];
      pdi = PrintFunctionDebugInfo /. opts /. Options[FieldAlgebra];
      pdil = PrintFunctionDebugInfoLevel /. opts /. Options[FieldAlgebra];
...
(* ============================================== *)
RAnalyzer[FullSol_] :=
    Module[{anInf, fita, ex, ey, ehFull, exy, anAngle, anParAmpl, anCrossAmpl, rotm, anTrf, exyOut, retval},
      anInf = SolutionGetReflectedAnalyzerInfo[FullSol];
      ...
      (*Print["exy = ",N[Chop[exy]],", exyOut = ",N[Chop[exyOut]],", RAnalyzer = ",N[retval],", Original = ",N[Abs[exy[[1,1]]]^2+Abs[exy[[2,1]]]^2],", anAngle = ",anAngle];*)
      Return[retval];
    ];
(* ============================================== *)
TAnalyzer[FullSol_] :=
    Module[{anInf, fita, ex, ey, ehFull, exy, anAngle, anParAmpl, anCrossAmpl, rotm, anTrf, exyOut, retval, opts, n1, n2, utll},
      anInf = SolutionGetTransmittedAnalyzerInfo[FullSol];
      opts = GetSolOptions[FullSol];
      anAngle = AnalyzerInfoGetAngle[anInf];
      anParAmpl = AnalyzerInfoGetParallelAmplitude[anInf];
      anCrossAmpl = AnalyzerInfoGetCrossedAmplitude[anInf];
      ...
      anTrf = {{anParAmpl, 0}, {0, anCrossAmpl}};exyOut = anTrf.rotm.exy;
      retval = Abs[exyOut[[1, 1]]]^2 + Abs[exyOut[[2, 1]]]^2;
      (*Print["exy = ",N[Chop[exy]],", exyOut = ",N[Chop[exyOut]],", TAnalyzer = ",N[retval],", Original = ",N[Abs[exy[[1,1]]]^2+Abs[exy[[2,1]]]^2],", anAngle = ",anAngle];*)
      Return[retval];
    ];
...
(* ============================================== *)
M2EValRe[evi_, FullSol_] :=
    Module[{retval, opts, evIdx, egsys, optsFunc},
...
(* ============================================== *)
ReEGE2[evi_, xyz_, FullSol_] :=
    Module[{retval, evIdx, ehFld, eFld, hFld, fldIdx, pntgS, ehMult},
      evIdx = Max[Min[evi, 4], 1];
...
(* ============================================== *)
PoyntingVector2[evi_, xyz_, FullSol_] :=
    Module[{retval, evIdx, ehFld, eFld, hFld, fldIdx, pntgS},
      evIdx = Max[Min[evi, 4], 1];
      fldIdx = Max[Min[xyz, 3], 1];
      ehFld = GetSolEHTEG[FullSol, evIdx];
      eFld = {ehFld[[1]], ehFld[[2]], ehFld[[3]]};
      ...
      Print["pntgS = ",N[pntgS]];Print["retval = ",N[retval]];*)
      Return[retval];
    ];
(* ============================================== *)
PoyntingI[xyz_, FullSol_] :=
    Module[{retval, evIdx, ehFld, eFld, hFld, fldIdx, pntgS},
      ehFld = GetSolIncidentLight[FullSol];
      fldIdx = Max[Min[xyz, 3], 1];
      eFld = {ehFld[[1]], ehFld[[2]], ehFld[[3]]};
      hFld = {ehFld[[4]], ehFld[[5]], ehFld[[6]]};
...
(* ============================================== *)
PoyntingR[xyz_, FullSol_] :=
    Module[{retval, evIdx, ehFld, eFld, hFld, fldIdx, pntgS},
      ehFld = GetSolReflectedLight[FullSol];
      fldIdx = Max[Min[xyz, 3], 1];
      eFld = {ehFld[[1]], ehFld[[2]], ehFld[[3]]};
      hFld = {ehFld[[4]], ehFld[[5]], ehFld[[6]]};
      pntgS = N[Re[Cross[eFld, Conjugate[hFld]]]];retval = pntgS[[fldIdx]];
...
(* ============================================== *)
PoyntingT[xyz_, FullSol_] :=
    Module[{retval, evIdx, ehFld, eFld, hFld, fldIdx, pntgS},
      ehFld = GetSolTransmittedLight[FullSol];
      fldIdx = Max[Min[xyz, 3], 1];
      eFld = {ehFld[[1]], ehFld[[2]], ehFld[[3]]};
      hFld = {ehFld[[4]], ehFld[[5]], ehFld[[6]]};
      pntgS = N[Re[Cross[eFld, Conjugate[hFld]]]];retval = pntgS[[fldIdx]];
...
Return[retval];];
(* ============================================== *)
PoyntingXY2[evi_, FullSol_] :=
    Module[{retval, pntgX, pntgY},
      pntgX = PoyntingVector2[evi, 1, FullSol];
      pntgY = PoyntingVector2[evi, 2, FullSol];
      retval = Sqrt[pntgX^2 + pntgY^2];
      Return[retval];
    ];
(* ============================================== *)
PoyntingXYPhase2[evi_, FullSol_] :=
    Module[{retval, pntgX, pntgY, fi, ro, sol, sol1, pntgXY},
      pntgX = PoyntingVector2[evi, 1, FullSol];
      pntgY = PoyntingVector2[evi, 2, FullSol];
...
(* ============================================== *)
EI[xyz_, FullSol_] :=
    Module[{retval, evIdx, ehFld, eFld, hFld, fldIdx},
      ehFld = GetSolIncidentLight[FullSol];
      fldIdx = Max[Min[xyz, 3], 1];
      eFld = {ehFld[[1]], ehFld[[2]], ehFld[[3]]};
      hFld = {ehFld[[4]], ehFld[[5]], ehFld[[6]]};
...
(* ============================================== *)
EpsComponent[UpDown_, ReIm_, xyzRow_, xyzCol_, FullSol_] :=
    Module[{retval, mtrx, row, col, Media, Film, len, mrtx},
      Media = GetSolMedia[FullSol];
      row = Max[Min[xyzRow, 3], 1];
      col = Max[Min[xyzCol, 3], 1];
      Film = MediaFilm[Media];
      len = MediaFilmLength[Media];

      If[UpDown > 0 && UpDown <= len,
        mrtx = FilmLayerEpsilon[Film[[UpDown]]], If[UpDown === -1,
        mrtx = MediaUpperEpsilon[Media], mrtx = MediaLowerEpsilon[Media]];
      ];
...
(* ============================================== *)
MuComponent[UpDown_, ReIm_, xyzRow_, xyzCol_, FullSol_] :=
    Module[{retval, mtrx, row, col, Media, len, Film, Film, len, mrtx},
      Media = GetSolMedia[FullSol];
      row = Max[Min[xyzRow, 3], 1];
      col = Max[Min[xyzCol, 3], 1];
      Film = MediaFilm[Media];
      ...
      If[UpDown > 0 && UpDown <= len,
        mrtx = FilmLayerMu[Film[[UpDown]]], If[UpDown === -1,
        mrtx = MediaUpperMu[Media], mrtx = MediaLowerMu[Media]];
      ];
...
(* ============================================== *)
RoComponent[UpDown_, ReIm_, xyzRow_, xyzCol_, FullSol_] :=
    Module[{retval, mtrx, row, col, Media, Film, len, mrtx},
      Media = GetSolMedia[FullSol];
      row = Max[Min[xyzRow, 3], 1];
      col = Max[Min[xyzCol, 3], 1];
      ...
      If[UpDown > 0 && UpDown <= len,
        mrtx = FilmLayerRo[Film[[UpDown]]], If[UpDown === -1,
        mrtx = MediaUpperRo[Media], mrtx = MediaLowerRo[Media]];
      ];
...
(* ============================================== *)
XitEGBase[evi_, FullSol_] :=
    Module[{retval, evIdx, ehField, opts, setElctSgn, useZAxis, elpCutOff, pdi, pdil},
      opts = GetSolOptions[FullSol];
      pdi = PrintFunctionDebugInfo /. opts /. Options[FieldAlgebra];
      pdil = PrintFunctionDebugInfoLevel /. opts /. Options[FieldAlgebra];
...
EltEG[evi_, FullSol_] :=
    Module[{evIdx, ehField, retval, opts, setElctSgn, useZAxis},
      opts = GetSolOptions[FullSol];
      setElctSgn = SetEllipticitySignT /. opts /. Options[FieldAlgebra];
      useZAxis = UseZAxisForEllipticityT /. opts /. Options[FieldAlgebra];
      evIdx = Max[Min[evi, 4], 1];
...
(* ============================================== *)
CalculateEllipsometricData[FullSol_] :=
    Module[{solBeta0, solBeta90, eBeta0fieldR, eBeta90fieldR, eBeta0fieldT, eBeta90fieldT, fita, retVal},
...
PsiPP[FullSol_] :=
    Module[{eld, retVal}, eld = CalculateEllipsometricData[FullSol];
...
MuellerMatrixT[FullSol_] :=
    Module[{eld, retVal},
      ...
      Return[retVal];];
(* ============================================== *)
CalculateStokesVector[eField : {_, _, _}, hField : {_, _, _}, inveseZcomp_?BooleanQ] :=
    Module[{stokesVector, kVec, eY, eXp, AxBx, AxBy, AyBx, AyBy, AVec, BVec, eXpC},
...
(* ============================================== *)
kVector[eField : {_, _, _}, hField : {_, _, _}, inveseZcomp_?BooleanQ] :=
    Module[{ehVec, ehAmpl, ehArg, kVec},
...
(* ============================================== *)
StokesVectorAllR[FullSol_] :=
    Module[{stokesVector, eField, hField},
...
StokesPolarized[stokesVector_] :=
    Module[{II, pp, MM, CC, SS},
...
StokesGamma[stokesVector_] :=
    Module[{Ip, pp, MM, CC, SS , gamma},
      MM = stokesVector[[2]];
      CC = stokesVector[[3]];
      SS = stokesVector[[4]];
...
StokesChi[stokesVector_] :=
    Module[{Ip, pp, MM, CC, SS , chi, sin2Gamma, cos2Gamma},
      MM = stokesVector[[2]];
      CC = stokesVector[[3]];
      SS = stokesVector[[4]];
...
(* ============================================== *)
StokesGammaDegreeR[stokesVector_] :=
    Module[{retVal},
...
(* ============================================== *)
StokesGammaR[FullSol_] :=
    Module[{retVal, stokesVector},
...
(* ============================================== *)
StokesChiDegreeR[stokesVector_] :=
    Module[{retVal},
...
(* ============================================== *)
StokesPolarizedR[stokesVector_] :=
    Module[{retVal},
