...
(* :Copyright: K^3, 2001-2015 *)
(* :Package Version: $ Revision: 5.03 $ $ Date:2015/03/20 $ *)
...
(*==============================================*)
Options[BerremanInverse] = {BerremanInverseVersion -> 5.03, LambdaMultiplier -> (1 / 10^9), AngleMultiplier -> Degree, DataStartRow -> 2, LambdaColumn -> 3, FitaColumn -> 4, BetaColumn -> 5, GammaColumn -> 6, EllipticityColumn -> 7, OutputStartColumn -> 10, OutputColumns -> {{strUseAllOutputColumns}, {strUseAllOutputColumns}}, MinizationMultiplier -> 10^9, UseStepMonitor -> True, UseEvaluationMonitor -> False, UseNMinimize -> False, EpsilonType -> BIEPSTYPESTDTWOSTEPTRAPSPARENT, AngleRoundingMultiplier -> 100, UseArbitraryPhase -> False, EpsilonLowerBoundary -> 1.9, EpsilonUpperBoundary -> 4, EpsilonNonDiagLowerBoundary -> -0.4, EpsilonNonDiagUpperBoundary -> 0.4, MFHlpDataSet -> 1, FitaTolerance -> (1 / 10^4), EpsilonIMLowerBoundary -> -BIIMTOLERANCE, EpsilonIMUpperBoundary -> 4, EpsilonIMNonDiagLowerBoundary -> -1, EpsilonIMNonDiagUpperBoundary -> 1, MFHlpAbsorbingEpsilon -> False, EpsilonStart -> EpsilonFromN[1.5, 1.9, 1.7], (* added in 4.02 *) PrintEpsilon -> True, EvaluationMonitorCount -> 1, StepMonitorCount -> 1, EvaluationMonitorSecondStepCount -> 1, StepMonitorSecondStepCount -> 1};
(*==============================================*)
(*EpsilonType:0-standard transparent (6 variables)*)
(*==============================================*)
ComplexUnitStepAND[cplxNum_] := Module[{retval}, (*Print["num = ",cplxNum];*)Return[UnitStep[Re[cplxNum]] * UnitStep[Im[cplxNum]]]];

ComplexUnitStepOR[cplxNum_] := Module[{retval}, retval = UnitStep[Min[Re[cplxNum]] + UnitStep[Im[cplxNum]], 1];
(*Print["num = ",cplxNum,", retval = ",retval];*)Return[retval]];

MFHlpSetParLimits[par_, minVal_, maxVal_, minIMVal_ : -BIIMTOLERANCE, maxIMVal_ : BIIMTOLERANCE] := Module[{retval}, retval = ComplexUnitStepOR[Complex[minVal, minIMVal] - par] + ComplexUnitStepOR[par - Complex[maxVal, maxIMVal]];
Return[retval];];

MFHlpSetEpsilonLimits[Epsilon : {{_, _, _}, {_, _, _}, {_, _, _}}, opts___] := Module[{epsLB, epsUB, epsUP, epsNDLP, epsNDLB, epsNDUB, epsIMLB, epsIMUB, epsIMUP, epsIMNDLP, epsIMNDLB, epsIMNDUB, retval}, epsLB = EpsilonLowerBoundary /. opts /. Options[BerremanInverse];
...
MFHlp[Epsilon : {{_, _, _}, {_, _, _}, {_, _, _}}, invSol_, mfoptsInpt___] := Module[{Media, sol, retval, len, incLght, lambda, fita, beta, gamm, funcLen, solArr, utll, solAvg, solArrAvg, opts, mfMult, n1, n2, nOut, h1, h2, epsSubtr, Film, Layer, MediaTrf, varList, flmLen, IsAverageArr, msDST, prtEps, avgtyp, funcArr, mfopts, ampl, elpct, useParallelTbl, solArray, inptArray},
  retval = 0;
...
(*==============================================*)
ProcessInverseOptions[opts___] := Module[{retval, lmbCol, fitaCol, betaCol, gammaCol, outStartCol, dtStart, lmbMult, angMult, outCols, fTolr, epsStart, optprc, elpctCol}, optprc = Flatten[{opts}];
...
(*==============================================*)
(* 1, 2, 3, 4, 5 - lambda, fita, beta, gamma, ellipticity columns, 6 - start output column, 7 - data start row, 8 - lambda multiplier,9 - angles multiplier, 10 - output columns in format {{...},{...}}, 11 - fitaTolerance, 12 - Start Epsilon *)
ProcessInputData[InptList_, FuncList_, IdxOpt_] := Module[{retval, len, inptRow, outptRow, outLen, row, startRow, rowLen, outColsLen1, outColsLen2, inpt1 = {}, outpt1 = {}, inpt2 = {}, outpt2 = {}, outCols, dstColLen, fl1 = {}, fl2 = {}, fTolr, outptRow2, elpctCol},
...
InverseSolutionNew[lambda_, RefrIndList : {_, _, _}, ThicknessList : {_, _}, InptList_, FuncList_, Description_ : "", opts___] := Module[{inptVar, outptFunc, retval, n1, n2, nOut, optprc, status, IdxOpt, inptData, h1, h2, epsStart}, Print["Initializing Inverse Problem..."];
n1 = RefrIndList[[1]];
n2 = RefrIndList[[2]];
nOut = RefrIndList[[3]];
h1 = ThicknessList[[1]];
...
(*==============================================*)
ProcessMinSolEps[Epsilon : {{_, _, _}, {_, _, _}, {_, _, _}}, InvSol_] := Module[{retval, egVal, epsRet, fi, theta, psi, edif, opts, solAng, MFA, FiThetaPsi, angMult},
  opts = InverseSolutionOptions[InvSol];
...
InverseSolutionRunStdTransparent[InvSol_] := Module[{retval, opts, stpMonFlg, evalMonFlg, nMin, epsTyp, minSol, evCnt, stpCnt, minVal, TimeStart, Epsilon, procEps, hlpVal, eps1, eps2, eps3, eps12, eps13, eps23, varList, lmbMult, varPointsList, varCondList, stpMon, evalMon, FuncName, meth, evalMonCnt},
  retval = InvSol;
  opts = InverseSolutionOptions[InvSol];
...
InverseSolutionRun[InvSol_] := Module[{retval, opts, stpMon, evalMon, nMin, epsTyp},
  opts = InverseSolutionOptions[InvSol];
  epsTyp = EpsilonType /. opts /. Options[BerremanInverse];
...
(*==============================================*)
InverseSolutionSave[InvSol_] := Module[{Calc, retval = 0}, Return[retval];];
(*==============================================*)
EpsilonComp[eps0_, deltaEps_, nu_] := Module[{retEps}, retEps = {{eps0 - deltaEps, nu, 0}, {nu, eps0 + deltaEps, 0}, {0, 0, eps0}};
Return[retEps];
];

EpsilonFull[eps0_, deltaEps_, nu_, eps13_, eps23_, eps33_] := Module[{retEps, eps11, eps22, eps12}, eps11 = eps0 - deltaEps + eps13^2 / eps33;
...
ISRunTwoStepStdTransparent[InvSol_] := Module[{MF, varList, varPointsList, FuncName, stpMon, evalMon, minSol, minVal, TimeStart = TimeUsed[], evCnt = 0, stpCnt = 0, n0Sol, deltaEpsSol, nuSol, mfopts, epsSol, procEps, retval, epsStart, nStart, n0Start, deltaEpsStart, nuStart, minSol1, minSol2, deltaEps33Sol, eps13Sol, eps23Sol, opts, evalMonCnt, stpMonCnt, evalMonCnt2, stpMonCnt2, evalMonCntHlp, stpMonCntHlp},
  ...
  *)
  ...
  Return[retval];];
(*==============================================*)
EpsilonCompHlp[n0_, deltaEps_, nu_, EpsIM1_, EpsIM2_, nuIM_] := EpsilonComp[n0^2 * Complex[1, (EpsIM1 + EpsIM2) / 2], n0^2 * Complex[deltaEps, (EpsIM1 - EpsIM2) / 2], n0^2 * Complex[nu, nuIM]];

EpsilonFullHlp[n0_, deltaEps_, nu_, deltaEps33_, eps13_, eps23_, EpsIM1_, EpsIM2_, nuIM_, EpsIM3_, EpsIM13_, EpsIM23_] := EpsilonFull[n0^2 * Complex[1, (EpsIM1 + EpsIM2) / 2], n0^2 * Complex[deltaEps, (EpsIM1 - EpsIM2) / 2], n0^2 * Complex[nu, nuIM], n0^2 * Complex[eps13, EpsIM13], n0^2 * Complex[eps23, EpsIM23], n0^2 * Complex[1 + deltaEps33, EpsIM3 + (EpsIM1 + EpsIM2) / 2]];
(*==============================================*)
Attributes[ISRunTwoStepStdAbsorbing] = {HoldFirst};
ISRunTwoStepStdAbsorbing[InvSol_] := Module[{MF, varList, varPointsList, FuncName, stpMon, evalMon, minSol, minVal, TimeStart = TimeUsed[], evCnt = 0, stpCnt = 0, n0Sol, deltaEpsSol, nuSol, mfopts, epsSol, procEps, retval, minSol1, EpsIM1Sol, minSol2, deltaEps33Sol, eps13Sol, EpsIM3Sol, EpsIM23Sol, nuIMSol, EpsIM2Sol, eps23Sol, EpsIM13Sol, opts, evalMonCnt, stpMonCnt, evalMonCnt2, stpMonCnt2, evalMonCntHlp, stpMonCntHlp},
  ...
  *)
  ...
  procEps = ProcessMinSolEps[epsSol, InvSol];
  (*retval[[4,1]]-Epsilon,retval[[4,2]]-{Angles}*)retval[[4, 1]] = procEps[[1]];retval[[4, 2]] = procEps[[2]];
  InvSol = retval;
  Return[retval];];
(*==============================================*)
