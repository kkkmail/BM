...
(* :Version: Revision: 6.02.001, Date: 2016/04/12 *)
...
(* ============================================== *)
Options[BerremanDirect]={BerremanDirectVersion -> 6.02,RotateAll->True,ConsecutiveRotation->True,PrintTimeEstimate->True,PrintCalculationProgress->False,PrintCalculationDetails->False,UseThickLastLayer->False,NoOfAveragingPoints->5,AveragingPeriods->1,AveragingType->BDAVGTYPESERIES,Amplitude->1,TransmittedAnalyzerAngle->0,TransmittedAnalyzerParallelAmplitude->1,TransmittedAnalyzerCrossedAmplitude->0,ReflectedAnalyzerAngle->0,ReflectedAnalyzerParallelAmplitude->1,ReflectedAnalyzerCrossedAmplitude->0,AnalyzerAngleAbsoluteValue->False,UseAnalyzer->False,UseMakeFileName->False,EigenValueIndex->1,UsePlotLabel3D->True,UseChop->True,ChopTolerance->10^-7,ChopPrecision->6,PrintFunctionDescription->True,IgnoreZeroIFullBase -> True, ZeroIFullBaseReplaceValue -> 1,Plot3DType -> Plot3D, BDPerformCalculations -> True, BDPlotFigures -> True,PlotOptions3D ->{PlotPoints ->25, PlotRange -> All}, PlotOptions2D ->{PlotPoints ->25, PlotRange -> All, MaxRecursion -> 3}};
...
(* ============================================== *)
ListPoints[lst_,1]:=Module[{len},len=Length[lst];Return[lst[[len]]]];
ListPoints[lst_,i_Integer]:=Module[{len},
len=Length[lst];
Return[lst[[len-i+1]]*ListPoints[lst,i-1]]
];
(* ============================================== *)
ListPointsFull[lst_]:=Module[{retval,len},
len=Length[lst];
...
(* ============================================== *)
GetIndexList[lst_,idx_Integer]:=Module[{retval,lsthlp,len},lsthlp=ListPointsFull[lst];len=Length[lsthlp];
retval=Table[1+GetQuotient[lsthlp,idx-1,i+1],{i,len}];
Return[retval];
];
(* ============================================== *)
GetNumberOfPoints[v_]:=Module[{retval},retval=Round[If[Length[v]>=3,1+If[v[[3]]!=0,((v[[2]]-v[[1]])/v[[3]]),0,0],1,1]];
Return[retval];
];
(* ============================================== *)
GetValue[v_,i_,UseMultiplier_:True]:=Module[{retval},
retval=If[Length[v]>=3,v[[1]]+(i-1)*v[[3]],If[Length[v]>=1,v[[1]],0,0],0]*If[UseMultiplier===True,If[Length[v]>=5,v[[5]],1,1],1];
Return[retval];
];
(* ============================================== *)
GetValueNumberOfPointsList[vlst_]:=Module[{retval,len},
len=Length[vlst];
retval=Table[GetNumberOfPoints[vlst[[i]]],{i,len}];
Return[retval];
];
(* ============================================== *)
GetValueList[vlst_,idx_,UseMultiplier_:True]:=Module[{retval,len,IdxLst,varlst},
...
TransformMedia[Media_,varlist_,opts___]:=Module[{rotnew,rotall,MediaTrf,FilmTrf,FilmHlp,fi,theta,psi,flmLen,gamm,rotgamm,consRot,n1,n2,nOut,Descr,h2,Film,len,rotn,lmbd,eps,mu,ro,eps1,mu1,ro1,eps2,mu2,ro2,epsVal,muVal,roVal,eps1Val,mu1Val,ro1Val,eps2Val,mu2Val,ro2Val, flm, flmLayer},
...
(* ============================================== *)
GetSol[Calc_,idx_]:=Module[{sol,Media,inclght,opts,MediaTrf,VarList,values,Ampl,pdi,pdil},
Media=Calc[[1]][[1]];
VarList=Calc[[1]][[2]];
opts=Calc[[2]];
...
(* ============================================== *)
GetSolAvg[Calc_,idx_]:=Module[{sol,Media,inclght,opts,MediaTrf,VarList,values,Ampl, elpct},
Media=Calc[[1]][[1]];
VarList=Calc[[1]][[2]];
opts=Calc[[2]];
values=GetValueList[VarList,idx];
...
GetInptLen[Calc_]:=Module[{TotalNumberOfPoints,Var,VarLen},
Var=Calc[[1]][[2]];
VarLen=Length[Var];
...
(* ============================================== *)
GetSolArr[Calc_,idx_,len_]:= Module[{retval, sol},
sol=GetSol[Calc,idx];
...
(* ============================================== *)
GetSolArrAvg[Calc_,idx_,len_]:=Module[{retval,sol},
sol=GetSolAvg[Calc,idx];
retval=Table[{sol},{len}];
Return[retval];
];
(* ============================================== *)
GetFuncList[Calc_]:=Module[{retval},
retval=Calc[[1]][[3]];
Return[retval];
];
(* ============================================== *)
GetFuncArr[Calc_,idx_]:=Module[{solArr,len,FuncList,retval,utll,solArrAvg,avgtyp,IsAverageArr,lstTbl,fnelhlp1,fnelhlp2,ueFuncListHlp1,solArrHlp1,solArrHlp2,opts,pdi,pdil},
...
(* ============================================== *)
GetInput[Calc_]:=Module[{retval,maxcnt,VarList},
...
(* ============================================== *)
GetOutput[Calc_]:=Module[{retval, maxcnt, useParallelTbl,clcOpts, CalcArr,CalcArrOut,idx},
maxcnt=GetInptLen[Calc];
...
VarListNew[IncidentLightInfo:{{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_,_}}]:=VarListNew[IncidentLightInfo,defaultCommonAnglesInfo];
(* ============================================== *)
VarListNew[IncidentLightInfo:{{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_,_}},CommonAnglesInfo:{{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_,_}}]:=Module[{retval},retval=Join[IncidentLightInfo,{{0,0,1,"Ellipticity"}},CommonAnglesInfo];
Return[retval];
];
(* ============================================== *)
VarListNew[IncidentLightInfo:{{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_}}]:=VarListNew[IncidentLightInfo,defaultCommonAnglesInfo];
(* ============================================== *)
VarListNew[IncidentLightInfo:{{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_}},CommonAnglesInfo:{{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_,_}}]:=Module[{retval},retval=Join[IncidentLightInfo,CommonAnglesInfo];
Return[retval];
];
(* ============================================== *)
VarListLambdaIdx=1;
...
VarListAddLayer[VarList_,LayerAngleInfo:{{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_,_},{_,_,_,_,_}}]:=Module[{retval},
...
CalcNew[Media_,VarList_,FuncList_,Description_:"",opts___]:=Module[{retval,optrul,uanz,len,f,optf,ua,FuncNameListHlp,FuncDescList},
...
CalcPerform[Calc_]:=Module[{retval,outpt,inpt,tstart,ttdiff,fulltstart,fulltend,prtCalcProgr,opts},
...
*)
(* ============================================== *)
GetFuncNameList[FuncList_,opts___]:=Module[{FuncNameList,len,lenFunc,idx,FuncName},
len=Length[FuncList];
...
(* ============================================== *)
GetFuncDescList[FuncList_,opts___]:=Module[{FuncDescList,len,lenFunc,idx,FuncDesc},
len=Length[FuncList];
...
Return[FuncDescList];];
(* ============================================== *)
CalcPlot[Calc_,pltOptsRaw___]:=Module[{FuncList,FuncNameList,len,clcOpts,prtfDesc,FuncDescList},
(*Print["CalcPlot"];*)
...
(* ============================================== *)
CalcPlotFunc[Calc_,Func_,FuncName_,pltOptsRaw___]:=Module[{f,xVar,xStart,xEnd,plotOpts,Media,varLst,vLen,xName,yName,xVarCnt,var,xAssigned,varNew,opts,nnVar,nnVarMax,funcLst,isNumericN, funcNameLst,fStr,execStr,plotOptsStr,s,ii},
...
FuncPlot[xVar_,nnVar_]:=Module[{varN,clc,outpt,retval,isNumericX,nn},
...
(* ============================================== *)
ChopValue[val_,opts___]:=Module[{retval,uc,cTol,cPrec},
...
(* ============================================== *)
CalcPlot3DFunc[Calc_,Func_,FuncName_,SwapXY_,pltOptsRaw___]:=Module[{f,xVar,yVar,xStart,xEnd,yStart,yEnd,plotOpts,Media,varLst,vLen,xName,yName,zName,xVarCnt,yVarCnt,var,xAssigned,yAssigned,varNew,opts,usePltLbl},
Media=CalcMedia[Calc];
varLst=CalcVarList[Calc];
vLen=Length[varLst];
...
CalcFuncPlot[CalcFunc_,xVarLst:{_,_,_},pltOptsRaw___]:=Module[{FuncList,FuncNameList,len,Calc},
...
CalcFuncPlotFunc[CalcFunc_,xVarLst:{_,_,_},Func_,FuncName_,pltOptsRaw___]:=Module[{f,Calc,xVar,xStart,xEnd,plotOpts,xName,yName,var},
...
CalcFuncPlot3DFunc[CalcFunc_,xVarLst:{_,_,_},yVarLst:{_,_,_},Func_,FuncName_,pltOptsRaw___]:=Module[{f,Calc,xVar,yVar,xStart,xEnd,yStart,yEnd,plotOpts,xName,yName,zName,var,usePltLbl},
(*Ok,CalcFunc must return Calc for each x and y*)
...
(* ============================================== *)
CalcPrintTimeEstimate[Calc_]:=Module[{opts,prtts,maxcnt,tstart,tlen,sol,Media,inclght,estTime,cdt,sttime,mult,multpoints,napt,utll,avgtyp},
...
(* ============================================== *)
CalcPrintRunTime[Calc_]:=Module[{tt,ttsec},
ttsec=Calc[[3,3]];
...
(* ============================================== *)
SolutionGetTransmittedAnalyzerInfo[sol_]:=Module[{retval=0,opts,inclght,beta,analyzerAngle,analyzerParAmpl,analyzerCrossAmpl,anglAbs},
...
(* ============================================== *)
SolutionGetReflectedAnalyzerInfo[sol_]:=Module[{retval=0,opts,inclght,beta,analyzerAngle,analyzerParAmpl,analyzerCrossAmpl,anglAbs},
opts=GetSolOptions[sol];
inclght=GetSolIncidentLightInfo[sol];
beta=IncidentLightBeta[inclght];
...
(* ============================================== *)
CalcCollectionNew[BaseDir_,BaseFileName_:"",Description_:""]:=Module[{retval},
retval={{ToString[BaseDir],ToString[BaseFileName],ToString[Description]},{}};
Return[retval];
];
(* ============================================== *)
CalcCollectionLength[coll_]:=Module[{},
Return[Length[coll[[2]]]];
];
(* ============================================== *)
GetFullName[coll_,fName_]:=Module[{fullName,len},
...
CalcCollectionAddCalc[coll_,Calc_,fName_:""]:=Module[{fullName},
fullName=GetFullName[coll,fName];
coll[[2]]=Join[coll[[2]],{{Calc,fullName}}];
Return[coll];
];
(* ============================================== *)
CalcCollectionItem[coll_,idx_Integer]:=Module[{},
Return[coll[[2]][[idx]][[1]]]
];
(* ============================================== *)
Attributes[CalcCollectionPerform]={HoldFirst};
CalcCollectionPerform[coll_]:=Module[{retval,len,Calc},
...
(* ============================================== *)
CalcCollectionSave[coll_,DoPrintOut_:False]:=Module[{retval,len},
retval=True;
len=CalcCollectionLength[coll];
...
(* ============================================== *)
CalcCollectionPlot[coll_,pltOptsRaw___]:=Module[{len,Calc},
len=CalcCollectionLength[coll];
Do[CalcPlot[coll[[2,idx,1]],pltOptsRaw],{idx,len}];
];
(* ============================================== *)
CalcCollectionPlot3D[coll_,SwapXY_,pltOptsRaw___]:=Module[{len,Calc},
len=CalcCollectionLength[coll];
Do[CalcPlot3D[coll[[2,idx,1]],SwapXY,pltOptsRaw],{idx,len}];
];

(* ============================================== *)
CalcCollCombine[coll_,funcNo_,hVarInputNo_:0,vVarInputNo_:0,OutputHeader_:False]:=Module[{retval,collLen,colRowLen,collOutArr,collRowArr},
...
(* ============================================== *)
CalcCollCombinePlot3D[coll_,funcNo_,hVarInputNo_:0,vVarInputNo_:0,OutputHeader_:False,pltOptsRaw___]:=Module[{collOutArr,xStart=0,xEnd=1,yStart=0,yEnd=1,xName="x",yName="y",zName="z",plotOpts},
...
(* ============================================== *)
SolAvgLambdaBoundaries[lambda_,fita_,n1_,n2_,LayerThickness_,opts___]:=Module[{retval,stp,lmb,dltlmb,lmbstart,lmbend,len,opt,utll,napt,avgper,avgtyp,psi,NumberOfWavelengths},
...
(* ============================================== *)
SolutionAverageNew[Media_,IncidentLight_,opts___]:=Module[{retval,lmbbound,lmb,len,lmbstart,stp,optprc,lambda,fita,beta,gamm,n1,n2,LayerThickness,Film,flmLen,inclgth,avgtyp},
optprc=Flatten[{opts}];
...
(* ============================================== *)
SSMakeFirstStep[Media_,IncidentLight_,opts___]:=Module[{retval,sol,solToAvg,inclNext,lambda,fita,n1,n2,ehZero,fita2,gamm,h2,PPPm,beta,pdi,pdil,utll},
...
(* ============================================== *)
SSMakeDownStep[Media_,IncidentLight_,opts___]:=Module[{retval,n1,n2,lambda,fita,sol,inclght,MediaBound,solCmb,ehZero,nOut,gamm,ehZeroRefl,h2,PPPm,Film,beta,solToAvg,inclNext,pdi,pdil,utll},
...
(* ============================================== *)
SSMakeUpStep[Media_,IncidentLight_,opts___]:=Module[{retval,inclFlp,MediaFlp,solFlp,ehZero,ehReflFlp,ehTrFlp,ehRefl,ehTr,lambda,fita,gamm,beta,solToAvg,inclNext,pdi,pdil,utll},
...
(* ============================================== *)
SolutionSumNew[Media_,IncidentLight_,optsRaw___]:=Module[{retval,opts,napt,Film,flmLen,lambda,fita,beta,gamm,n1,n2,nOut,solFirst,len,solStp,MediaBound,incLght,pdi,pdil, optsNew,ampl,ehField},
...
AveragingFunc[f_,solavg_,IsAverage_:True]:=Module[{len,retval=0,navg,optf,naIdx=1,fnelhlp1,fnelhlp2,ueFuncListHlp1,solArrHlp1,solArrHlp2, hlpArr,hlpArrI,hlpArrR,hlpArrT,pdi,pdil,opts,retval1,val,FullSol,fita,eField,hField,ehVec,kVec,ehAmpl,ehArg,afc,param},
...
(* ============================================== *)
MakeFileName[prefix_,Media_,thk_,opts___]:=Module[{retval,strFileName,Film,flmLen,eps,nMatr,kMatr},strFileName=prefix;
